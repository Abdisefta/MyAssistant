...
(Get-Content 'C:\b\android\app\build.gradle') -replace 'versionCode 19','versionCode 20' -replace 'versionName "1.7.3"','versionName "1.7.4"' | Set-Content 'C:\b\android\app\build.gradle'
(Get-Content 'C:\b\android\app\build.gradle') -replace 'versionCode 18','versionCode 20' -replace 'versionName "1.7.2"','versionName "1.7.4"' | Set-Content 'C:\b\android\app\build.gradle'
(Get-Content 'C:\b\android\app\build.gradle') -replace 'versionCode 17','versionCode 20' -replace 'versionName "1.7.1"','versionName "1.7.4"' | Set-Content 'C:\b\android\app\build.gradle'
...
$logPath = 'C:\b\build-174.log'
Set-Content -Path $logPath -Value "BUILD 174 START $(Get-Date -Format o)"
$buildOk = $false
for ($attempt = 1; $attempt -le 40; $attempt++) {
  "BUILD ATTEMPT $attempt" | Add-Content $logPath
  $attemptLog = "C:\b\build-174-attempt-$attempt.txt"
...
if ($buildOk -and (Test-Path $apk)) {
  Copy-Item $apk 'C:\Users\user\Desktop\MyAssistant174.apk' -Force
  'KLAR 174' | Add-Content $logPath
} else {
