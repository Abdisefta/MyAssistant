...
$gradle = $gradle -replace 'versionCode\s+\d+', 'versionCode 41'
$gradle = $gradle -replace 'versionName\s+"[^"]+"', 'versionName "1.9.5"'
...
$logPath = 'C:\b\build-195.log'
Set-Content -Path $logPath -Value "BUILD 195 START $(Get-Date -Format o)"
$buildOk = $false
for ($attempt = 1; $attempt -le 40; $attempt++) {
  "BUILD ATTEMPT $attempt" | Add-Content $logPath
  $attemptLog = "C:\b\build-195-attempt-$attempt.txt"
...
if ($buildOk -and (Test-Path $apk)) {
  Copy-Item $apk 'C:\Users\user\Desktop\MyAssistant195.apk' -Force
  Write-Host 'KLAR: C:\Users\user\Desktop\MyAssistant195.apk'
} else {
  Write-Host 'BYGG MISSLYCKADES — se C:\b\build-195.log'
}
