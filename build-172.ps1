...
(Get-Content 'C:\b\android\app\build.gradle') -replace 'versionCode 17','versionCode 18' -replace 'versionName "1.7.1"','versionName "1.7.2"' | Set-Content 'C:\b\android\app\build.gradle'
...
$logPath = 'C:\b\build-172.log'
Set-Content -Path $logPath -Value "BUILD 172 START $(Get-Date -Format o)"
$buildOk = $false
for ($attempt = 1; $attempt -le 30; $attempt++) {
  "BUILD ATTEMPT $attempt" | Add-Content $logPath
  $attemptLog = "C:\b\build-172-attempt-$attempt.txt"
...
if ($buildOk -and (Test-Path $apk)) {
  ...
  'KLAR 172' | Add-Content $logPath
  Copy-Item $logPath 'C:\Users\user\Desktop\build-172.log' -Force -ErrorAction SilentlyContinue
} else {
