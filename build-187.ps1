...
$gradle = $gradle -replace 'versionCode\s+\d+', 'versionCode 33'
$gradle = $gradle -replace 'versionName\s+"[^"]+"', 'versionName "1.8.7"'
...
$logPath = 'C:\b\build-187.log'
Set-Content -Path $logPath -Value "BUILD 187 START $(Get-Date -Format o)"
$buildOk = $false
for ($attempt = 1; $attempt -le 40; $attempt++) {
  "BUILD ATTEMPT $attempt" | Add-Content $logPath
  $attemptLog = "C:\b\build-187-attempt-$attempt.txt"
...
if ($buildOk -and (Test-Path $apk)) {
  Copy-Item $apk 'C:\Users\user\Desktop\MyAssistant187.apk' -Force
  Write-Host 'KLAR: C:\Users\user\Desktop\MyAssistant187.apk'
} else {
  Write-Host 'BYGG MISSLYCKADES â€” se C:\b\build-187.log'
}
