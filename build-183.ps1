...
$gradle = $gradle -replace 'versionCode\s+\d+', 'versionCode 29'
$gradle = $gradle -replace 'versionName\s+"[^"]+"', 'versionName "1.8.3"'
...
$logPath = 'C:\b\build-183.log'
Set-Content -Path $logPath -Value "BUILD 183 START $(Get-Date -Format o)"
$buildOk = $false
for ($attempt = 1; $attempt -le 40; $attempt++) {
  "BUILD ATTEMPT $attempt" | Add-Content $logPath
  $attemptLog = "C:\b\build-183-attempt-$attempt.txt"
...
if ($buildOk -and (Test-Path $apk)) {
  Copy-Item $apk 'C:\Users\user\Desktop\MyAssistant183.apk' -Force
  'KLAR 183' | Add-Content $logPath
  Write-Host 'KLAR: C:\Users\user\Desktop\MyAssistant183.apk'
} else {
  'MISSLYCKAD' | Add-Content $logPath
  Write-Host 'BYGG MISSLYCKADES — se C:\b\build-183.log'
}
