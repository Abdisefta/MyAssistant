...
$gradle = $gradle -replace 'versionCode\s+\d+', 'versionCode 34'
$gradle = $gradle -replace 'versionName\s+"[^"]+"', 'versionName "1.8.8"'
...
if ($buildOk -and (Test-Path $apk)) {
  Copy-Item $apk 'C:\Users\user\Desktop\MyAssistant188.apk' -Force
  Write-Host 'KLAR: C:\Users\user\Desktop\MyAssistant188.apk'
} else {
  Write-Host 'BYGG MISSLYCKADES â€” se C:\b\build-188.log'
}
