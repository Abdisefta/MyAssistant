...
$gradle = $gradle -replace 'versionCode\s+\d+', 'versionCode 40'
$gradle = $gradle -replace 'versionName\s+"[^"]+"', 'versionName "1.9.4"'
...
$logPath = 'C:\b\build-194.log'
Set-Content -Path $logPath -Value "BUILD 194 START $(Get-Date -Format o)"
$buildOk = $false
for ($attempt = 1; $attempt -le 40; $attempt++) {
  "BUILD ATTEMPT $attempt" | Add-Content $logPath
  $attemptLog = "C:\b\build-194-attempt-$attempt.txt"
...
if ($buildOk -and (Test-Path $apk)) {
  Copy-Item $apk 'C:\Users\user\Desktop\MyAssistant194.apk' -Force
  Write-Host 'KLAR: C:\Users\user\Desktop\MyAssistant194.apk'
} else {
  Write-Host 'BYGG MISSLYCKADES — se C:\b\build-194.log'
}
