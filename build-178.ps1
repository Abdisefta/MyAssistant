...
$gradle = $gradle -replace 'versionName\s+"[^"]+"', 'versionName "1.7.8"'
...
$logPath = 'C:\b\build-178.log'
Set-Content -Path $logPath -Value "BUILD 178 START $(Get-Date -Format o)"
$buildOk = $false
for ($attempt = 1; $attempt -le 40; $attempt++) {
  "BUILD ATTEMPT $attempt" | Add-Content $logPath
  $attemptLog = "C:\b\build-178-attempt-$attempt.txt"
...
if ($buildOk -and (Test-Path $apk)) {
  Copy-Item $apk 'C:\Users\user\Desktop\MyAssistant178.apk' -Force
  'KLAR 178' | Add-Content $logPath
  Write-Host 'KLAR: C:\Users\user\Desktop\MyAssistant178.apk'
} else {
  'MISSLYCKAD' | Add-Content $logPath
  Write-Host 'BYGG MISSLYCKADES — se C:\b\build-178.log'
}
